...
ber_mmse = zeros(scenario_num, points);
...
% Compute the ber
[ber_zf(index_sim,:), ber_vblast(index_sim,:), ber_mmse(index_sim,:)] = com_sys(numTx(index_sim), numRx(index_sim), num_symbols, M, eb_n0);
...
hold on;
semilogy(eb_n0,ber_mmse(index_sim,:),'b-o');
hold off
